...
<body id="students">
	...
	<main style="min-height:480px">
		<div class="container">
		  <s:form action="list" id="btn" namespace="/kprc/school" method="post" enctype="multipart/form-data">
				...
				<table class="table">
					...
					<tbody>
						<s:iterator value="pageModel.list" status="st">
							<tr>
								<td><s:a action="schooldetail" namespace="/kprc/student"><s:param name="student.id">${id}</s:param>${name}</s:a></td>
								<td><s:if test='gender=="M"'>男</s:if><s:else>女</s:else></td>
								...
								<td><s:a action="examinedetail" class="btn btn-xs btn-primary" namespace="/kprc/school"><s:param name="student.id">${id}</s:param>查看</s:a></td>
								<td><s:a action="updateStatusA" class="btn btn-xs btn-primary" namespace="/kprc/school"><s:param name="student.id">${id}</s:param>通过</s:a></td>
								<td><s:a action="updateStatusB" class="btn btn-xs btn-primary" namespace="/kprc/school"><s:param name="student.id">${id}</s:param>拒绝</s:a></td>	
							</tr>
						</s:iterator>
					</tbody>
				</table>
...
<script type="text/javascript">
 /*   function prese(){
		$('#btn').attr("action", '<s:url action="add" namespace="/kprc/student"/>');
		$('#btn').submit();
	}
