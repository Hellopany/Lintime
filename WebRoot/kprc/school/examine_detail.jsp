 ...
 <div class="container">
    ...
    <table class="table">
				<tbody>
					...
					<s:iterator value="activityList" status="st">
					<tr>
						<th>活动名称</th>
						<td>${name}</td>
						<th>开始时间</th>
		...
		<s:form action="" id="btn">
				<s:hidden name="student.id"></s:hidden>
		</s:form>
        <div class="form-group">
            <div class="col-sm-4 col-sm-offset-4">
                <button type="button" class="btn btn-primary" onclick="return btnA()"><i class="glyphicon glyphicon-ok" onclick="return btnA()"></i> 通过</button>
            	<button type="button" class="btn btn-primary" onclick="return btnR()"><i class="glyphicon glyphicon-ok" onclick="return btnR()"></i> 拒绝</button>
            </div>
        </div>
    </main>
...
var national = [
    ...
    function btnA(){
       if (confirm("确定通过该修改吗？")){
		$('#btn').attr("action", '<s:url action="updateStatusA" namespace="/kprc/school" />');
		$('#btn').submit();
		}
	};
    function btnR(){
        if (confirm("确定拒绝该修改吗？")){
			$('#btn').attr("action", '<s:url action="updateStatusB" namespace="/kprc/school" />');
			$('#btn').submit();
		}
	};
</script>
