	...
	<main style="min-height:480px">
		<div class="container">
			<div class="page-header">
				...
				<s:form action="modify" id="btn">
					<s:hidden name="student.id"></s:hidden>
					<s:if test='Status!="M"'><button type="submit" class="btn btn-default">申请修改</button></s:if>
					<s:if test='Status=="M"'><button type="button" class="btn btn-primary" onclick="return updateStatus()"><i class="glyphicon glyphicon-ok" onclick="return updateStatus()"></i> 提交</button></s:if>
					<s:else><a onclick="return myinfo()" class="btn btn-default">返回</a></s:else>
				</s:form>
				</div>
				<h1>学生信息</h1>
				<h5>状态:
				<s:if test='Status=="I"'>保存</s:if>
				<s:if test='Status=="M"'>修改 未提交</s:if>
				<s:if test='Status=="S"'>修改 已提交</s:if>
				<s:if test='Status=="A"'>审批 通过</s:if>
				<s:if test='Status=="R"'>审批 拒绝</s:if>		
				</h5>
			</div>
			<table class="table">
				<tbody>
					...
					<tr>
						...
						<th>民族</th>
						<td id="national">${student.nation}</td>
					</tr>
					<tr>
						<th>籍贯</th>
					...
					<tr>
						<th class="bg-info" colspan="6">获得荣誉信息</th>
					</tr>
...
var national = [
    ...
    function updateStatus(){
	   if (confirm("您确定提交本次修改吗？")){
			$('#btn').attr("action", '<s:url action="updateStatus" namespace="/kprc/student"/>');
			$('#btn').submit();
		}
	}
	function myinfo(){
			$('#btn').attr("action", '<s:url action="detail" namespace="/kprc/student"/>');
			$('#btn').submit();
	}
</script>
</body>
